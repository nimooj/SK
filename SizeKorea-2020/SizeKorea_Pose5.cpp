...
std::vector<std::string> LandmarkPose5 = {
	"엉덩이돌출점",				// 0
	"배돌출점",					// 1
	"엉덩이최대둘레수준점",			// 2
	"왼쪽주먹가운데점",			// 3
...
std::vector<std::string> ResultPose5 = {
	"엉덩이최대둘레높이",			// 0
	"굽힌팔꿈치높이",				// 1
	"엉덩이둘레",					// 2
	"엉덩이최대둘레",				// 3
...
/*
*	\brief	오른쪽 팔꿈치아래점을 찾는 함수
*
*	\param	pMesh[in]	대상 메쉬에 대한 포인터
*
*	\return	오른쪽 팔꿈치아래점의 위치를 반환한다.
*/
EgVertex *SK_Pose5_Bottom_Olecranon(EgMesh *pMesh)
{
	for (int i = 0; i < 10; ++i)
	{
		...
		double Len = theSlicer.CvxSlice(pMesh, 2, 0.7, -1.0, 0.0, 0.0, 1.0, -1.0, 1.0);
		if (Len != -1.0)
			break;
	}
...
*	\param	pMesh[in]	대상 메쉬에 대한 포인터
*
*	\return	왼쪽 등부위돌출점의 좌표를 반환한다.
*/
EgVertex *SK_Pose5_Left_Back_Protrusion(EgMesh *pMesh)
{
	// 메쉬를 절단한다.
	double Len = theSlicer.CvxSlice(pMesh, 2, 0.7, 0.0, 1.0, 0.0, 1.0, 0.0, 1.0);
	if (Len == -1.0)
	{
	...
	pResult = SK_Extreme_Vertex(pResult, 30.0, 5, 2);
	return pResult;
}
...
EgVertex *SK_Pose5_Right_Acromion(EgMesh *pMesh)
{
	...
	{
		double z = 0.7 + 0.3 * i / 9.0;
		double Len = theSlicer.CvxSlice(pMesh, 2, 0.7, 0.0, 1.0, 0.0, 1.0, -1.0, 1.0);
		if (Len != -1.0)
			break;
	}
